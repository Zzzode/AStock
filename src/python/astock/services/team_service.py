...
class TeamAnalysisService:
    ...
    async def analyze(
        ...
        try:
            ...
            global_profile = await self.feedback_learner.get_global_profile(user_id=user_id)
    ...
    def _recommend_roles(self, question: str) -> list[str]:
        ...
        roles = ["core"]
        role_keywords = {
            "short_term": ["short-term", "scalp", "intraday", "day trade", "overnight", "t+0"],
            "swing": ["swing", "pullback", "rebound", "trend recovery"],
            "long_term": ["long-term", "value", "fundamentals", "valuation", "earnings", "dividend"],
            "sentiment": ["sentiment", "hot topic", "theme", "capital flow", "news", "public opinion"],
        }
    ...
    def _build_orchestration_plan(
        ...
        core_agents = [self._build_agent_spec(spec, packet_ready) for spec in CORE_AGENT_SPECS]
        ...
        return {
            ...
            "lead_rules": [
                "Python only provides the data packet; it does not output buy/sell conclusions.",
                "Consume the packet first, then decide whether to fetch missing data.",
                "Final analysis and action recommendations are generated by the Agent team.",
            ],
            "warnings": warnings,
        }

    def _build_agent_spec(self, spec: dict[str, Any], packet_ready: bool) -> dict[str, Any]:
    ...
    def _build_data_quality(
        ...
        return {
            ...
            "feedback": {
                ...
                "quality": "learned" if stock_profile.get("sample_count", 0) else "cold_start",
            },
        }
    ...
    def _build_warnings(
        ...
        if quote_quality == "daily_only":
            warnings.append("Real-time quote degraded to latest trading day daily snapshot.")
        elif quote_quality == "snapshot_degraded":
            warnings.append("Real-time quote is a degraded snapshot; order book fields may be incomplete.")
        elif quote_quality == "unavailable":
            warnings.append("Real-time quote unavailable; analysis based on historical data only.")

        if screen_payload.get("error"):
            warnings.append("Strategy screening failed; only basic quote and technical data available.")

        if int(stock_profile.get("sample_count", 0) or 0) == 0:
            warnings.append("User feedback profile is empty; no personalized preference adjustment applied.")
    ...
    def _save_session_report(
        ...
        report_path = self.sessions_dir / f"team-{result.code}-{result.analyzed_at:%Y%m%d}.md"
