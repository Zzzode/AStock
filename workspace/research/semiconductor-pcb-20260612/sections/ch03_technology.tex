...
承接图表3-10的工艺节点，设备耗材链上的关键工步（钻孔、LDI曝光、电镀填孔、蚀刻/检测）所对应的高阶参数——孔径比、线宽线距、mSAP精细度、微钻规格、镀层厚度——恰恰是公开资料最缺失的字段。材料股能否给出连续Dk/Df与ASP、板厂能否给出客户级层数与孔结构、载板厂能否给出分产品良率，分别决定后续估值对材料股、板厂、载板厂施加多大折价。因此图表3-11不再罗列``未收集到的公开信息''，而是把这些缺口直接映射为每类公司的估值折价触发器。
...
为支撑上表中``CCL介电常数/损耗因子''这一行的判断，下文图表3-12至图表3-18按物理顺序排列，构成一条可按序校验的证据链：图表3-12给出官方TDS/Line Up多频点Dk/Df参数点（证明材料性能方向），图表3-13给出收入/毛利代理（证明商业化兑现），图表3-14给出隐含单位经济性代理（证明单位收入/单位成本可观察），图表3-15给出交期与备货周期代理（证明供需紧张），图表3-16列明仍需外部确认的字段（定义折价边界），图表3-17给出客户认证时间线（证明导入进展），图表3-18收束已覆盖的官方技术能力。逻辑校验顺序为``参数（3-12）→ 收入/毛利与单位经济性（3-13/3-14）→ 交期（3-15）→ 外部数据请求（3-16）→ 认证（3-17）→ 官方能力（3-18）''；读者可按此顺序校验``技术期权→盈利兑现''的兑现程度，三家公司证据强弱差异即对应基准/上修区间/触发器的分层。下文先按``参数与商业化证据''（图表3-12至图表3-15）和``认证时间线、外部数据缺口与已覆盖官方能力''（图表3-16至图表3-18）两个子节组织，便于把证据与缺口分开读。
...
上一子节确认了材料``能否进入高端应用、是否已经转为盈利''，本子节进一步给出导入进展（认证时间线）与仍未确认的字段（外部数据请求清单），用于界定折价边界并收敛本章的技术能力覆盖范围。
