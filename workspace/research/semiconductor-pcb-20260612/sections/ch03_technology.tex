...
AI服务器对PCB和CCL的改变不是线性的面积或层数增加，而是整个价值分布的重构。要判断一家公司在AI链条上的议价权，必须先理解高层板、HDI、背板/中板、M7--M10材料、ABF/BT/FC-BGA和制程设备之间的层级关系。这些技术变量决定了后续的公司估值和客户链判断的基础。
...
mSAP & 改良型半加成工艺（modified Semi-Additive Process），用于制造精细线路的高阶HDI和类载板PCB。 & 线宽线距越精细，工艺门槛和良率挑战越高，对应高端AI板和CoWoP类结构。 \\
正交背板 & 一种省去中板、直接正交连接的高速背板架构，以更高层数PCB替代部分铜缆互联。 & 层数和加工难度显著提升，利好高层板能力强的PCB厂和低损耗CCL。 \\
CoWoP & Chip on Wafer on PCB，打破封装基板与PCB边界、使PCB直接承担部分封装基板功能的架构。 & 单GPU配套PCB价值量显著提升，是高端PCB的结构性上修机会，但落地节奏和良率仍存不确定性。 \\
\bottomrule
\end{tabularx}
\end{exhibitbox}

名词表的一个直接推论是：上述节点并非同等重要，A股公司真正能``定价''的只有两条主线——OAM/UBB/背板/交换机板所需的``高层板 + HDI + 良率''组合（决定能否承接高端板级订单），以及M7--M10所需的``低损耗CCL + 客户认证''组合（决定材料厂能否进入AI/算力供应链）。ABF/BT/FC-BGA载板和CBF/玻璃基板更多是``良率 + 折旧''驱动的期权，需要更长验证周期才能转为利润；CPO/光互联则是反向的替代变量。这一分层是后续价值池迁移图（图表3-4）和材料代际映射（图表3-8）的读图基础，也对应估值章``可定价高端产能 vs 仅跟踪扩产项目''的折价口径。

从整机服务器看，芯片/HBM是价值最高的核心；封装/载板是芯片和系统之间的高密度连接；OAM、UBB、背板和交换机板是服务器内部和柜内高速互联；PCB、CCL、设备和耗材则是承接这些高速互联的制造与材料底座。公开券商框架曾估算AI服务器CCL价值约4000--5000元/台，但这只是历史框架——该框架来自未具名公开券商框架（data room未取得原文，仅作量级参考，证据等级E2/E3；分项数字来自同框架的不同拆解口径，GPU板组含OAM及周边、CPU主板含板载组件，分项之间存在交叉重叠，不可直接相加得总价值），不是当前GB300/Rubin平台的确认值。GB300/Rubin因CoWoP、正交背板（如78层PCB替代铜缆，公开券商产业框架说法，非公司确认）等架构变化，单位CCL/PCB价值可能与H100代际显著不同，当前并无公开确认值。注意单位口径：此处``元/台''指整机组装单台CCL总价值，与后续图表3-14``元/平方米/元/米/元/张''（CCL出厂单位售价）是不同口径，不可直接换算或混用。
...
把不同技术环节的价值来源与整机价值链对齐，可以提炼出一个可投资的判断：在AI服务器里，A股PCB/CCL链条吃到的不是``芯片价值''，而是``芯片之后的连接价值''，并且这些连接价值进一步向两个方向集中——高端板级互联（OAM/UBB/背板）依赖层数与良率门槛，先进封装（ABF/BT/FC-BGA）依赖载板良率与折旧消化。CPO/玻璃基板则是反向变量，需要在后续估值里单列替代风险而非一并外推。从投资角度看，价值是沿着``材料+工艺+认证''方向集中，而不是随单机PCB面积线性增长——这是理解后续价值池迁移的关键。

\section{价值池迁移示意}

上一节从技术变量排序和名词定义出发，本节用结构图把价值流向可视化——芯片平台升级如何逐层传导到板级互联、材料、设备耗材和对应公司。读这张图需要区分主线（实线箭头）和输入线（虚线箭头）的差别：主线是价值从芯片向系统侧逐层传导的方向，输入线是材料、加工、设备耗材作为价值承载者切入的投入路径。核心读法是：越靠近芯片侧的环节越吃技术门槛和良率的钱，越靠近制造端越依赖材料配方和工艺稳定性决定份额与认证。
...
\draw[inputarrow] (materials) -- (names2);
\draw[inputarrow] (process) -- (boards);
\draw[inputarrow] (process) -- (switch);
\draw[inputarrow] (process) -- (names1);
...
\draw[decorate, decoration={brace, amplitude=4pt}, thick, color=navy] (6.0,0.12) -- (6.0,-2.68);
\node[label, anchor=west] at (6.2,-2.88) {层数越高、孔结构越复杂，\\加工良率和客户认证门槛越高。};
\end{tikzpicture}
\end{exhibitbox}
...
\midrule
OAM/GPU模组板 & 高阶HDI/高层板，材料等级高；未具名公开券商H100框架中OAM的CCL价值约1745元（data room未取得原文，仅作量级参考，E2/E3）。 & 高端HDI良率、低损耗材料等级、客户认证。 \\
UBB统一基板 & 面积大、层数高，承担多GPU互联；未具名公开券商H100框架中UBB的CCL价值约1364元（千元级，量级参考，E2/E3）。 & 层数、孔结构、良率和产能利用率。 \\
交换机/背板 & 高速低损耗材料要求高，对M7--M10 CCL和阻抗控制敏感。 & CCL价格、交期、背板/中板结构。 \\
CCL单机价值 & 公开券商历史框架估算AI服务器CCL价值约4000--5000元/台（未具名公开券商H100平台框架，data room未取得原文，仅作量级参考，证据等级E2/E3；分项数字来自同框架的不同拆解口径，GPU板组含OAM及周边、CPU主板含板载组件，分项之间存在交叉重叠，不可直接相加得总价值）。 & 该数值为H100代际历史框架，不能直接视为当前GB300/Rubin确认值；GB300/Rubin因CoWoP、正交背板等架构变化单位价值可能显著不同。 \\
\bottomrule
\end{tabularx}
\sourcenote{未具名公开券商H100平台框架（data room未取得原文，仅作量级参考，E2/E3）；公司公开披露与AStock产业整理}
\end{exhibitbox}
...
材料迁移不是简单替代，而是认证周期和配方稳定性竞争。生益科技、南亚新材、华正新材更直接暴露于M8/M9/M10认证、价格和交期，但三者证据强度不同：CCL材料股依赖认证节奏与产品级ASP（仍以代理为主），PCB板厂依赖层数/良率交付，载板依赖折旧曲线，因此不能套用同一估值权重——材料股按``参数确认 + 认证进展 + 毛利率验证''三段式触发分层，板厂按平台收入与良率分层，载板按良率与折旧消化分层。
...
\midrule
生益科技 & 官方IR显示AI服务器低损耗CCL需求提升，国内外终端GPU/AI项目合作中，部分产品已批量供应（IR原文措辞为``部分产品''，非平台级/客户级批量确认，仅作方向性参考）。 & E1/E2 & 材料龙头，估值取决于M8/M9收入占比、涨价持续性和生益电子AI板兑现。 \\
...
\bottomrule
\end{tabularx}
\sourcenote{公司官方IR、年报/半年报、问询函回复与互动易披露；AStock产业分类}
\end{exhibitbox}

材料代际之上，先进封装进一步重构芯片到板级的互联层级。当HBM与GPU通过2.5D/3D封装集成，当CoWoP类结构把封装功能下沉到PCB，传统``芯片—载板—系统板''的三层边界开始模糊。对投资分析来说，这意味着不能只按``PCB厂做板、CCL厂做材料、载板厂做封装基板''的旧框架切分价值，而要看每家公司在这条从芯片到系统的连接链条上能切入哪一层、良率和折旧能不能消化、客户认证走到哪一步。下一张关系图把先进封装、载板材料、玻璃基板替代和CPO风险放在同一张图上，便于快速定位每家A股公司的机会与风险所在。
...
\node[role] (drillrole) at (2.4,-1.95) {钻针耗材\\鼎泰高科};
\node[role] at (4.8,-1.05) {电镀药水/铜球\\（无A股标的—）};
\node[role] at (7.2,-1.05) {直写光刻\\芯碁微装};
\node[role] at (9.6,-1.05) {蚀刻液/检测设备\\（无A股标的—）};
\draw[dashed, color=accentblue] (drillrole) -- (drill);
\end{tikzpicture}
\end{exhibitbox}

设备耗材链的收益顺序通常滞后于PCB厂和材料厂，但在扩产周期中会放大资本开支弹性。生益科技和华正新材是材料供应商，不是层压设备商；大族数控对应钻孔设备，芯碁微装对应直写光刻/曝光设备，鼎泰高科对应微钻和刀具耗材（图中以虚线连回钻孔工步以表明其服务钻孔）；电镀药水/铜球、蚀刻液/检测设备两步当前无对应A股标的，以``—''标注。本图只标注有对应A股标的的工步。

\section{技术参数缺口}

前两节建立了技术变量排序、价值池迁移方向和材料/工艺/认证三条主线。本节转向一个更实际的问题：公开资料中，哪些工程参数已经可以验证、哪些仍然缺失，以及这些缺口如何影响估值。以下先用一个资料边界声明把已覆盖和未覆盖的范围划清，再用一组表把缺口映射为具体的估值折价触发器。
...
从PCB制造的工艺链条看，钻孔、LDI曝光、电镀填孔、蚀刻/检测等关键工步所对应的高阶参数——孔径比、线宽线距、mSAP精细度、微钻规格、镀层厚度——恰恰是公开资料最缺失的字段。材料股能否给出连续Dk/Df与ASP、板厂能否给出客户级层数与孔结构、载板厂能否给出分产品良率，分别决定后续估值对材料股、板厂、载板厂施加多大折价。因此，技术参数缺口不是“信息收集不全”的问题，而是直接对应估值折价的幅度和解除条件。
...
\bottomrule
\end{tabularx}
\sourcenote{基于公司公开披露、官方IR和AStock产业参数缺口评估；非公司确认值}
\end{exhibitbox}
...
CCL介电常数与损耗因子是材料股的核心定价变量，但参数本身不等于利润。从“材料性能达标”到“盈利兑现”之间有一条完整的传导链条：先看官方Dk/Df参数是否进入高端应用区间，再看收入与毛利率是否体现产品升级，再看单位经济性与供需紧张度是否支撑价格，最后看客户认证和官方能力是否确认可持续性。每缺一环，估值就要多打一层折价。

三家材料公司在这条传导链条上的位置不同：生益科技参数覆盖最全但分代收入不透明，南亚新材价格与订单支持率证据最硬但M9/M10仍在认证，华正新材参数对标高端但收入体量和客户认证最薄。这种差异直接对应估值章中基准情景、上修区间和触发器的分层。以下先从参数和商业化证据看起，再推进到认证时间线和仍未公开的缺口。
...
行业成本 & 电子布/铜箔 & 不适用。 & 花旗转载图片OCR显示电子布1080/2116/7628年内ASP分别+95\%/+91\%/+60\%（OCR识别值，未经原文/二级源校验，仅作方向性参考）；生益年报称Low DK布需求挤压薄型玻纤布产能、价格每月跳升10\%--20\%，铜箔价格急升。 & 供应商级采购价、CCL ASP传导时滞和公司级采购成本序列。 \\
\bottomrule
\end{tabularx}
\sourcenote{公司官网产品页与官方TDS/Line Up；公司2025年年度报告/半年报与问询函回复；存档原始券商报告与OCR价格线索——转载与OCR价格信号仅为线索，非公司级ASP序列}
\end{exhibitbox}

官方材料参数的可验证程度，决定了“材料方向正确”到底是叙事还是有数据支撑。生益已取得官网详情页、TDS和Line Up多频点Dk/Df表，南亚取得官网和PDF规格书Dk/Df点，华正取得官网高速材料表和HSD8/HSD8(K) Line-up多频点Dk/Df表。但参数可验证仍不能替代价格序列或收入模型——以南亚为例，官方问询函披露覆铜板单位售价由104.97元升至113.13元、在手订单支持率由81.10\%升至137.52\%，是最接近“价格传导验证”的公开代理，但仍不是M8/M9/M10 ASP。对材料股估值，统一采用“参数确认 + 认证进展 + 毛利率验证”的三段式触发（见估值章对应折价参数），而不是直接把M9/M10叙事转成收入占比。
...
行业成本 & 不适用。 & 花旗转载图片OCR显示电子布1080/2116/7628年内ASP分别+95\%/+91\%/+60\%（OCR识别值，未经原文/二级源校验，仅作方向性参考）。 & 这是投入品价格线索，不是公司CCL ASP；还缺采购价、传导时滞和公司级报价序列。 \\
\bottomrule
\end{tabularx}
...
\bottomrule
\end{tabularx}
\sourcenote{公司年度报告/问询函中的收入与销量字段；销量原始字段见 \texttt{sources/official-annual-core-20260615/600183-生益科技2025年年度报告.md}（覆铜板16062.79万平方米、粘结片22254.51万米）与 \texttt{sources/official-annual-core-20260615/603186-浙江华正新材料股份有限公司2025年年度报告-修订版.md}（覆铜板3886.83万张）；代理证据（proxy only），非产品级ASP}
\end{exhibitbox}
...
\subsection{认证时间线、外部数据缺口与已覆盖官方能力}

参数、收入和交期证据解决了“材料能不能进高端、赚钱效应有没有起来”的问题，但还需要回答两个更直接影响估值的问题：一是客户认证走到哪一步、何时能放量，二是还有哪些关键数据没有公开、这些缺口对应多大的估值折价。认证进度决定收入兑现的时间点，数据缺口决定折价的幅度，两者共同框定了材料股的跟踪节奏和上修/下修触发器。
...
鹏鼎控股 & HKEX文件显示产品覆盖AI加速卡、AI服务器、数据中心交换机、UBB和光模块；70层以上MLPCB量产、100层以上技术能力；24层6+12+6 HDI和28层8+12+8 HDI量产；官方IR确认800G/1.6T光模块SLP量产出货、3.2T研发中。 & 具名客户/平台收入、产品级Dk/Df、线宽线距和客户BOM。 \\
