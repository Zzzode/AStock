...
为支撑上表中``CCL介电常数/损耗因子''这一行的判断，下文图表3-12至图表3-17构成一条证据链，按``参数确认 → 认证进展 → 收入/毛利代理 → 单位经济性代理 → 交期与供需代理 → 外部数据缺口''顺序排列：图表3-12给出官方TDS/Line Up多频点Dk/Df参数点（证明材料性能方向），图表3-17给出客户认证节点（证明导入进展），图表3-13和图表3-14给出收入、毛利与单位售价代理（证明商业化兑现），图表3-15给出交期与备货周期代理（证明供需紧张），图表3-16则列明仍需外部确认的字段（定义折价边界）。读者可按此顺序校验``技术期权→盈利兑现''的兑现程度，三家公司证据强弱差异即对应基准/上修区间/触发器的分层。下文先按``参数与商业化证据''（图表3-12至图表3-15）和``认证时间线与外部数据缺口''（图表3-16至图表3-18）两个子节组织，便于把证据与缺口分开读。
