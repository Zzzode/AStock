...
\begin{exhibitbox}[图表3-1b：关键名词解释]
\footnotesize
...
从整机服务器看，芯片/HBM是价值最高的核心；封装/载板是芯片和系统之间的高密度连接；OAM、UBB、背板和交换机板是服务器内部和柜内高速互联；PCB、CCL、设备和耗材则是承接这些高速互联的制造与材料底座。公开券商框架曾估算AI服务器CCL价值约4000--5000元/台，但这只是历史框架，不是当前GB300/Rubin平台的确认值。

\begin{exhibitbox}[图表3-1c：技术差异与整机价值链位置]
\footnotesize
...
\begin{exhibitbox}[图表3-2：AI平台到PCB/CCL价值池迁移]
\centering
\begin{tikzpicture}[
...
这张图的投资含义是：算力芯片升级不会平均分配价值。芯片和HBM价值最高，但A股PCB链条主要承接的是芯片之后的连接价值：封装载板负责芯片到系统板的高密度连接，OAM/UBB/背板/交换机板负责服务器内部和柜内高速互联，低损耗CCL、高层PCB、钻孔/曝光设备和微钻刀具共同支撑这些连接。只有当公司披露收入、认证、产能利用率和良率时，这些技术节点才可以进入估值模型。

\begin{exhibitbox}[图表3-3：HDI与高多层板层叠示意]
\centering
\begin{tikzpicture}[
...
\begin{exhibitbox}[图表3-4：AI服务器板级价值池和单位经济性]
\centering
\footnotesize
...
\midrule
OAM/GPU模组板 & 高阶HDI/高层板，材料等级高；公开券商框架中GPU板组是AI服务器CCL价值的重要部分。 & 高端HDI良率、低损耗材料等级、客户认证。 \\
UBB统一基板 & 面积大、层数高，承担多GPU互联；公开框架中UBB价值量约千元级。 & 层数、孔结构、良率和产能利用率。 \\
交换机/背板 & 高速低损耗材料要求高，对M7--M10 CCL和阻抗控制敏感。 & CCL价格、交期、背板/中板结构。 \\
CCL单机价值 & 公开券商历史框架估算AI服务器CCL价值约4000--5000元/台。 & 该数值为历史框架，不能直接视为当前GB300/Rubin确认值。 \\
\bottomrule
\end{tabularx}
\end{exhibitbox}
...
\begin{exhibitbox}[图表3-5：M6--M10高频高速材料迁移]
\centering
\begin{tikzpicture}[
...
\begin{exhibitbox}[图表3-6：材料代际和公司映射]
\footnotesize
...
南亚新材 & 官方材料显示M6/M7稳定量产，M8及以下通过部分海外终端认证，M9/M10推进认证。 & E1 & 高速材料储备明确，但需区分PCB客户收入和终端平台收入。 \\
华正新材 & 公开资料显示112/224Gbps材料、CBF/BT材料和高等级CCL项目，但具名客户和收入拆分仍缺。 & E1/E2 & 国产材料期权，需用订单和利润修复验证。 \\
\bottomrule
\end{tabularx}
\end{exhibitbox}

\begin{exhibitbox}[图表3-7：先进封装与载板材料关系]
\centering
\begin{tikzpicture}[
...
替代风险来自两个方向。第一，玻璃基板如果在高端封装中扩大应用，可能吸收部分原本属于有机载板或高端材料的价值；第二，光互联/CPO如果减少板级电信号传输距离，可能降低部分背板、交换机板和低损耗CCL的单机用量。换句话说，先进封装既能增加载板和材料机会，也可能把部分传统PCB/CCL价值转移到封装侧或光互联侧。因此，后续估值不能只按“AI服务器单机PCB价值持续上升”外推，还要跟踪玻璃基板、CPO和CoWoP类结构的采用节奏。

\begin{exhibitbox}[图表3-8：PCB制造流程与材料/设备/耗材映射]
\centering
\begin{tikzpicture}[
...
\begin{exhibitbox}[图表3-9：未披露工程参数与估值影响]
\footnotesize
...
图表3-9的核心作用是把技术不确定性转成估值动作：沪电、胜宏、鹏鼎的上修需要平台收入、层数/孔结构和良率证据；生益、南亚、华正的上修需要M8/M9/M10收入占比、ASP和认证证据；深南、兴森的上修需要载板良率、折旧和客户认证证据。没有这些触发器时，技术方向可以支持跟踪优先级，但不应直接进入基准EPS或目标估值。

\begin{exhibitbox}[图表3-9b：材料参数与价格序列补充]
\scriptsize
...
\bottomrule
\end{tabularx}
\sourcenote{\texttt{data/material\_parameter\_price\_sequence\_20260618.md}; \texttt{sources/probe-material-parameters-20260618/}; company websites / official filings / IR records / archived broker text; repost and OCR price signals are clues, not company-level ASP series}
\end{exhibitbox}

新增材料序列把“材料方向正确”推进到“部分参数可验证”：生益已取得官网详情页、TDS和Line Up多频点Dk/Df表，南亚取得官网和PDF规格书Dk/Df点，华正取得官网高速材料表和HSD8/HSD8(K) Line-up多频点Dk/Df表。但是它还不能替代价格序列或收入模型。对材料股估值，当前应使用“参数确认 + 认证进展 + 毛利率验证”的三段式触发，而不是直接把M9/M10叙事转成收入占比。

\begin{exhibitbox}[图表3-9c：材料收入和毛利代理序列]
\scriptsize
...
\bottomrule
\end{tabularx}
\sourcenote{\texttt{data/material\_revenue\_margin\_proxy\_sequence\_20260618.md}; official filings / inquiry responses / archived OCR clues; proxy evidence only, not product ASP}
\end{exhibitbox}

材料股的判断因此可以分成两层：第一层是Dk/Df和认证，证明产品能不能进入高端应用；第二层是收入、毛利率和原材料/售价传导，证明技术升级是否已经转为盈利。南亚的数据最接近“价格传导验证”，生益的数据最适合观察材料龙头的利润弹性，华正的数据仍偏产品验证和小批量订单，估值折价应最高。

\begin{exhibitbox}[图表3-9e：隐含单位经济性代理]
\scriptsize
...
\bottomrule
\end{tabularx}
\sourcenote{\texttt{data/material\_implied\_unit\_economics\_proxy\_20260618.md}; official revenue and shipment/volume fields; proxy only}
\end{exhibitbox}
...
\begin{exhibitbox}[图表3-9f：交期与供应紧张代理]
\scriptsize
\begin{tabularx}{\textwidth}{L{1.8cm}X X}
\toprule
...
\bottomrule
\end{tabularx}
\sourcenote{\texttt{data/material\_leadtime\_supply\_tightness\_20260618.md}; official filings / inquiry responses; third-party lead-time claims excluded from confirmed evidence}
\end{exhibitbox}

交期和供应紧张证据决定材料价格能否持续。南亚已经有官方订单支持率和备货周期证据，说明景气并非只停留在叙事；生益和华正仍主要是管理层对交付和供应链能力的定性描述。后续如果不能取得分材料代际交期、配额或排产数据，M8/M9/M10的估值贡献仍应保持折价。

\begin{exhibitbox}[图表3-9g：材料线外部数据请求清单]
\scriptsize
\begin{tabularx}{\textwidth}{L{1.8cm}X X}
\toprule
...
\bottomrule
\end{tabularx}
\sourcenote{\texttt{data/material\_external\_data\_request\_pack\_20260618.md}; request checklist only, not evidence}
\end{exhibitbox}

这张请求清单定义了材料线下一步的外部工作。只要这些字段仍未取得，材料股只能用公开参数、认证、收入/毛利代理和交付代理来定性跟踪，不能把M8/M9/M10直接写成可建模的客户/平台EPS贡献。

\begin{exhibitbox}[图表3-9d：材料客户认证时间线]
\scriptsize
...
\bottomrule
\end{tabularx}
\sourcenote{\texttt{data/material\_customer\_certification\_timeline\_20260618.md}; official filings / IR / issuer interaction; certification timeline only, not revenue or ASP}
\end{exhibitbox}
...
\begin{exhibitbox}[图表3-10：已覆盖的官方技术能力]
\footnotesize
...
\bottomrule
\end{tabularx}
\sourcenote{\texttt{data/official\_technology\_parameter\_summary.md}; \texttt{data/official\_filing\_segment\_summary.md}; \texttt{data/official\_ir\_refresh\_20260616.md}; \texttt{data/current\_cninfo\_interaction\_sweep\_20260618.md}}
\end{exhibitbox}
