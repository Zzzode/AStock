...
| Market data & valuation | `data/raw_market_data.md`, `data/verified_market_data.md`, `analysis/valuation_model.md` | Current prices, market caps, PE/PB/PS, historical valuation bands, peer comparison, 7 global peers | Covered — market data verified against multiple sources | 实时日内数据不可得，使用日度收盘价 |
...
| Valuation model | `analysis/valuation_model.md` | PE/PS/PEG/EV-EBITDA multi-method valuation, tiered valuation system | Covered — comprehensive valuation framework | DCF 对周期股适用性有限；使用情景分析替代 |
| Valuation audit | `analysis/valuation_audit.md` | PE/PS/PB/PEG 数学校验，TTM 口径验证，估值方法论审计 | Covered — 估值算术验证已完成，PS 全部准确，PE 经 TTM 口径验证 | PB 因缺乏期末净资产数据无法精确复算；斯达半导 2025 财务数据缺失无法独立验证 |
...
| LaTeX report & PDF | `main.tex`, `sections/*.tex`, `main.pdf` | 100 页机构级研报，11 章节 + 附录，高盛风格 | Covered — full PDF report compiled v2.0 | 部分 B 级质量优化待持续迭代 |
...
Total chapter count: 11 chapters + appendix (up from 10). Page count: 100 pages (up from 48).
...
- 39 PASS / 0 FAIL verifier status maintained
