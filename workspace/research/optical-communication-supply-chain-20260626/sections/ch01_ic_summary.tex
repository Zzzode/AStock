\section{结论先行}
本报告重写后的核心结论是：光通信产业链必须按完整上下游理解，不能只看 AI 光模块。上游材料/基底决定光纤、激光芯片和调制器的物理边界；制造设备决定外延、硅光、主动耦合和测试良率；光芯片和光器件决定 1.6T/CPO 的战略稀缺性；光模块决定云厂商 AI capex 到收入利润的最直接弹性；光纤光缆、网络设备和下游应用决定运营商、DCI、FTTH 与企业网络的慢周期底盘。基于 2026-06-26 收盘后价格和 2026Q1 业绩折算，本报告覆盖 26 只可估值标的，另列产业链观察池。组合权重内在价值锚空间为 -47.8\%，市场共识调整后综合空间为 \textbf{-23.1\%}。
