...
\node[above=8pt, font=\bfseries\small, color=navy] at (7.4, {102*\scl}) {2026E 五大原厂合计（Samsung\slash SK\slash Micron\slash Kioxia+WD\slash YMTC+CXMT） >\$1010 亿};
...
% Source note
\node[below=0.4cm, font=\tiny, color=mutedgrey, align=center, text width=12cm] at (5.2, -1.8) {Vendor CC official disclosure [S-01/S-02/S-11] · YMTC\slash CXMT AStock model est. · 2027E vendor-unconfirmed};
\end{tikzpicture}
}
\end{exhibitbox}
...
\node[below=0.8cm of B, font=\scriptsize, color=steelgrey, text width=12.5cm, align=center, yshift=-0.2cm] {核心驱动：供给 20\% bit growth << 需求 30-35\% bit growth → 2026 年 Q3 起结构性短缺扩大。DRAM 基准缺口 \textcolor{navy}{\bfseries[--6\%\,~\,--8\%]} ↔ 合约价 QoQ \(+8\sim+10\%\)（悲观/乐观情景见 §10.2）。\textbf{BLOCK 治理}：C 级供需预测仅使用区间，严禁单点值。};
\end{tikzpicture}
}
\end{exhibitbox}
