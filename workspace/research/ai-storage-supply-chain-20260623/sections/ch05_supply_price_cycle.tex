...
% Y-axis grid & labels (0-180, step 20)
...
% X-axis: 5 years, positions at x=0.8, 3.0, 5.2, 7.4, 9.6
% 2023 (x=0.8): 40+18+9+7+5=79 → scales: 1 unit = 13.5B → 5.85
% 2024 (x=3.0): 45+20+10+8+6=89 → 6.59
% 2025 (x=5.2): 52+25+13+9+7=106 → 7.85
% 2026E (x=7.4): 62+32+18+11+9=132 → 9.78
% 2027E (x=9.6): 70+40+25+14+12=161 → 11.93
% Scale factor: 180 units → 13.32 cm.  1\$B = 13.32/180 = 0.074cm
\def\scl{0.074}
% --- 2023 ---
...
% Source note (inside tikz for layout)
...
\pgfmathsetmacro\hY{9*\scl}   \node[bar, ya, minimum height=\hY cm] at (7.4, \hS+\hK+\hM+\hW) {};
\node[below, font=\scriptsize, text=navy, font=\bfseries] at (7.4, -0.2) {2026E};
\node[above, font=\scriptsize\bfseries, color=navy] at (7.4, {132*\scl+0.12}) {132 ★};
...
\pgfmathsetmacro\hY{12*\scl}  \node[bar, ya, minimum height=\hY cm] at (9.6, \hS+\hK+\hM+\hW) {};
\node[below, font=\scriptsize] at (9.6, -0.2) {2027E};
\node[above, font=\scriptsize\bfseries] at (9.6, {161*\scl+0.12}) {161};
...
\node[above=8pt, font=\bfseries\small, color=navy] at (7.4, {132*\scl}) {2026E 三大原厂 Capex >\$1200 亿};
...
\begin{tabularx}{\linewidth}{>{\bfseries}l >{\raggedleft}p{1.1cm} >{\raggedleft}p{1.4cm} >{\raggedleft}p{0.9cm} >{\raggedleft}p{0.9cm} >{\raggedright\arraybackslash}X}
\toprule
...
\begin{tikzpicture}[
  sup/.style={rectangle, rounded corners=2pt, draw=accentblue, fill=accentblue!12, text width=3.2cm, align=center, minimum height=1.0cm, font=\bfseries\small},
  bal/.style={rectangle, rounded corners=3pt, draw=navy, fill=navy!18, text width=2.6cm, align=center, minimum height=1.1cm, font=\bfseries\small, text=white},
  dem/.style={rectangle, rounded corners=2pt, draw=riskred, fill=riskred!12, text width=3.2cm, align=center, minimum height=1.0cm, font=\bfseries\small},
...
% Demand side
\node[dem] (D1) at (11.6, 3.3) {AI 服务器 · HBM + DDR5\\\small 单机存储 +80\% YoY};
...
\node[dem] (D5) at (11.6, -1.5) {国内云 + 国产 AI 芯片\\\small 受管制补偿需求 +25\%};
...
\node[below=0.8cm of B, font=\scriptsize, color=steelgrey, text width=11cm, align=center, yshift=-0.2cm] {核心驱动：供给 20\% bit growth << 需求 30-35\% bit growth → 2026Q3 起结构性短缺扩大。DRAM 基准缺口 \textcolor{navy}{\bfseries[--6\%\,~\,--8\%]} ↔ 合约价 QoQ \(+8\sim+10\%\)（悲观/乐观情景见 §10.2）。\textbf{BLOCK 治理}：C 级供需预测仅使用区间，严禁单点值。};
\end{tikzpicture}
}
\end{exhibitbox}
...
\begin{tabularx}{\linewidth}{c >{\centering}p{1.5cm} >{\centering}p{1.5cm} >{\centering}p{1.6cm} >{\centering}p{1.6cm} >{\raggedright\arraybackslash}X}
\toprule
