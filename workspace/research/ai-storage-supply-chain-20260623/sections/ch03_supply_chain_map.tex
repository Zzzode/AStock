...
\node[ashar, fill=white, draw=riskamber, text=deepnavy, right=0cm of L1.east, anchor=east] (A1) {\faStar\enspace A 股 · 设备\\北方华创 · 中微\\拓荆 · 沪硅 · 南大};

% Layer 2
\node[layer=nvgreen, below=1.9cm of L1] (L2) {\faCogs\enspace L2 · 存储设计 \quad[\;8--10\%\;]};
...
\node[ashar, fill=white, draw=riskamber, text=deepnavy, right=0cm of L2.east, anchor=east] (A2) {\faStar\enspace A 股 · 设计\\澜起科技 (RCD 70\%)\\兆易 · 芯原 · 北京君正};

% Layer 3
\node[layer=riskamber, below=1.6cm of L2] (L3) {\faMicrochip\enspace L3 · 晶圆制造 \quad[\;38--42\%\;] \textcolor{riskred}{—— 价值最大环节}};
...
\node[ashar, fill=white, draw=riskamber, text=deepnavy, right=0cm of L3.east, anchor=east] (A3) {\faStar\enspace A 股间接\\\small A 股无直接原厂\\北方华创等为供应商};

% Layer 4
\node[layer=riskred, below=1.6cm of L3] (L4) {\faLayerGroup\enspace L4 · 封装 \quad[\;先进 12--15\% \;|\; 传统 5--7\%\;]};
...
\node[ashar, fill=white, draw=riskamber, text=deepnavy, right=0cm of L4.east, anchor=east] (A4) {\faStar\enspace A 股 · 封测\\长电 (XDFOI)\\通富 (AMD 绑定) · 华天};

% Layer 5
\node[layer=deepnavy, below=1.6cm of L4] (L5) {\faMemory\enspace L5 · 模组/主控 \quad[\;8--10\%\;]};
...
\node[ashar, fill=white, draw=riskamber, text=deepnavy, right=0cm of L5.east, anchor=east] (A5) {\faStar\enspace A 股 · 模组\\江波龙 (Foresee\/Lexar)\\深科技 (沛顿) · 佰维存储};

% Layer 6
\node[layer=navy, below=1.6cm of L5] (L6) {\faServer\enspace L6 · 整机集成 \quad[\;-\;]};
...
\node[ashar, fill=white, draw=riskamber, text=deepnavy, right=0cm of L6.east, anchor=east] (A6) {\faStar\enspace A 股 · 系统\\(本报告不覆盖)\\浪潮 · 联想 · 华为};

% Layer 7
\node[layer=steelgrey, below=1.6cm of L6] (L7) {\faGlobe\enspace L7 · 下游应用 (Capex 来源)};
...
\node[node, fill=steelgrey!10, right=of C2, text width=4.1cm] (C3) {消费电子 + 汽车\\\textgreater{} \$10000 亿/年 TAM\\PC 复苏 + AI PC + 新能源};
...
% Key stat — placed below L7 content row to avoid overlapping layer titles
\node[draw=navy, fill=white, rounded corners=3pt, font=\scriptsize\bfseries, text=navy, below=0.5cm of C2.south, anchor=north, inner sep=4pt] {\faExclamationTriangle\enspace A 股无原厂 · 配置聚焦上游/接口/封测};
\end{tikzpicture}
}
\end{exhibitbox}
...
\midrule
\rowcolor{nvgreen!5}
\textbf{存储设计·接口} & 8--10 & 55--70 & Rambus · Marvell & 约3--5 & 澜起科技 \astockstar & \textbf{全球领先无代差}——RCD 全球 70\% 份额，CXL 首发；A 股唯一「一阶 Alpha」。兆易 NOR 前三，DRAM 追赶。代差 0。 \\
\midrule
