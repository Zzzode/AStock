...
全球存储产业链从上游到下游可划分为七层：上游设备与材料、存储设计、晶圆制造、封装 (先进+传统)、模组与主控、整机集成、下游应用 (AI/云/终端)。A 股映射的核心机会集中在设备材料 (二阶 Beta)、接口芯片设计 (澜起 CXL 一阶 Alpha) 与先进封装 (长电/通富 弹性) 三层。
...
\begin{tikzpicture}[
  ...
  ashar/.style={rectangle, rounded corners=2pt, draw=riskamber, fill=riskamber!8, text width=2.35cm, align=center, minimum height=0.95cm, font=\scriptsize\bfseries, color=navy, inner sep=2pt, line width=0.7pt},
...
\node[ashar, right=of U4] (A1) {\faStar\enspace A 股 · 设备\\北方华创 · 中微\\拓荆 · 沪硅 · 南大};
...
\node[ashar, right=of D3] (A2) {\faStar\enspace A 股 · 设计\\澜起科技 (RCD 70\%)\\兆易 · 芯原 · 北京君正};
...
\node[ashar, right=of M2] (A3) {\faStar\enspace A 股间接\\\small A 股无直接原厂\\北方华创等为供应商};
...
\node[ashar, right=of P2] (A4) {\faStar\enspace A 股 · 封测\\长电 (XDFOI)\\通富 (AMD 绑定) · 华天};
...
\node[ashar, right=of MO2] (A5) {\faStar\enspace A 股 · 模组\\江波龙 (Foresee/Lexar)\\深科技 (沛顿) · 佰维存储};
...
\node[ashar, right=of S2] (A6) {\faStar\enspace A 股 · 系统\\(本报告不覆盖)\\浪潮 · 联想 · 华为};
...
% Key stat
\node[draw=navy, fill=white, rounded corners=3pt, font=\scriptsize\bfseries, text=navy, right=0.2cm of U2, yshift=-0.55cm] {aExclamationTriangle A 股无原厂 · 配置聚焦上游/接口/封测};
\end{tikzpicture}
}
\end{exhibitbox}
\sourcenote{产业链架构设计参考 Industry Landscape §1.3 mermaid 原版；价值占比来自中金 2026Q1 半导体深度报告 + AStock 交叉验证。}

\vspace{0.4em}
\noindent\textbf{投资含义}：产业链七层价值分布呈现显著的「哑铃型」——晶圆制造 (38-42\%) 和设备材料 (17-21\%) 合计占据近六成价值。A 股恰恰在晶圆制造层缺失 (YMTC/CXMT 未上市)，但在设备材料和先进封装两个高价值增量层拥有全球竞争力的标的。这一结构决定了 A 股存储链投资必须走「上游二阶 Beta + 接口芯片一阶 Alpha」的路径，而非追逐不存在的「国产 HBM 原厂」概念。
...
\centering
\footnotesize
\begin{tabularx}{\linewidth}{>{\raggedright}p{1.6cm} >{\raggedleft}p{0.9cm} >{\raggedleft}p{0.9cm} >{\raggedright}p{2.2cm} >{\raggedleft}p{0.9cm} >{\raggedleft}p{1.2cm} >{\raggedright\arraybackslash}X}
\toprule
\textbf{环节} & \textbf{价值占比(\%)} & \textbf{全球龙头毛利率(\%)} & \textbf{全球龙头} & \textbf{国产化率(\%)} & \textbf{A 股核心标的} & \textbf{A 股卡位与代差评估} \\
\midrule
\rowcolor{accentblue!5}
\textbf{半导体设备} & 10--12 & 45--60 & Lam · AMAT · ASML & {\faExclamationTriangle} 10--15 & \textbf{北方华创 (002371)\\中微公司 (688012)\\拓荆科技 (688072)} & 刻蚀/薄膜/ALD 在成熟制程全面突破，<16nm 验证中；\textbf{二阶 Beta 首选层}，国产化率仍有翻倍空间。代差 1--2 代。 \\
\midrule
\rowcolor{accentblue!5}
\textbf{半导体材料} & 7--9 & 35--50 & ShinEtsu · JSR & {\faExclamationTriangle} 8--12 & \textbf{沪硅产业 (688126)\\南大光电 (300346)} & 12 寸硅片 28nm 批量供应；ArF 光刻胶 2026 目标突破；KrF/特气已破。代差 1--3 代 (按品类)。 \\
\midrule
\rowcolor{nvgreen!5}
\textbf{存储设计·接口} & 8--10 & 55--70 & Rambus · Marvell & 约3--5 & \textbf{澜起科技 (688008)★} & \textbf{全球领先无代差}——RCD 全球 70\% 份额，CXL 首发；A 股唯一「一阶 Alpha」。兆易 NOR 前三，DRAM 追赶中。代差 0。 \\
\midrule
\textbf{存储设计·原厂} & 8--10 & 40--60 & SK · Samsung · Micron & <1 & \textbf{无直接标的} & YMTC/CXMT 未上市；兆易自研 DRAM 试产；A 股原厂层缺失是最大结构缺口。代差 3--4 代 (HBM)。 \\
\midrule
\textbf{晶圆制造} & 38--42 & 30--50 & Samsung · SK · YMTC & {\faExclamationTriangle} 8--15 & \textbf{无直接标的} & 长存/长鑫未上市，A 股仅通过设备供应商间接映射；DRAM 代差 1.5--2 代，NAND 代差 1 代。 \\
\midrule
\rowcolor{riskred!5}
\textbf{先进封装 (CoWoS)} & 12--15 & 25--35 & TSMC (约80\%) & {\faExclamationTriangle} 5--8 & \textbf{长电科技 (600584)\\通富微电 (002156)} & TSMC 垄断 HBM 封装产能；长电 XDFOI/通富 CoWoS 2027 前难撼份额；\textbf{弹性标的}，但 AI 实际贡献 <3\%。代差 1--2 代。 \\
\midrule
\textbf{传统封测} & 5--7 & 15--22 & ASE · Amkor & 约25--30 & \textbf{长电 / 通富 / 华天} & 华天等传统封装国产替代较充分，毛利率低。代差 <1 代。 \\
\midrule
\rowcolor{deepnavy!5}
\textbf{存储模组/SSD} & 8--10 & 10--20 & Smart Global · ATP & {\faExclamationTriangle} 10--20 & \textbf{江波龙 (301308)\\深科技 (000021)} & 深科技 Kingston 代工模式确定性高；江波龙 Foresee 企业级 SSD 导入头部云厂。代差 0--1 代 (技术门槛相对低)。 \\
\bottomrule
\end{tabularx}
\end{exhibitbox}
