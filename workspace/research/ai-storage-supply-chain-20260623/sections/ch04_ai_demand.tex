...
% HBM3E
\node[gen] at (-0.15, 4.2) {\textbf{HBM3E} \textcolor{riskred}{\small \ensuremath{\bigstar}\ 2026 主力}};
...
% 国产 HBM (虚线提示)
\node[gen, text=riskamber] at (-0.15, 0.8) {\textcolor{riskamber}{\faExclamationTriangle 国产 HBM} \textcolor{riskred}{市场提前 2 年!}};
...
% Markers
\draw[->, thick, riskred] (11.5, 0.2) -- node[above, font=\tiny, color=riskred] {\textbf{BIS 管制关键节点}} (11.5, 1.4);
\node[draw=deepnavy, fill=deepnavy!5, rounded corners=2pt, font=\scriptsize, inner sep=2pt, anchor=west, text width=10cm] at (0.2, 0.2) {ASP：HBM2E \$8--12/GB · HBM3E \$18--25/GB · HBM4 {\faExclamationTriangle}\$33--45/GB (初期)};
\end{tikzpicture}
}
\end{exhibitbox}
\sourcenote{HBM 参数：SK Hynix 2026 年 Q1 CC + NVIDIA Blackwell 官方规格 [S-03/S-11]；HBM4/HBM4E 路线：SK Hynix 2025 Investor Day + {\faExclamationTriangle} 行业基准预测 [S-12/S-19]；国产节奏：AStock 制程代差倒推评估。}
...
\centering
\footnotesize
\begin{tabularx}{\exhibitboxwidth}{>{\bfseries}c >{\raggedleft}p{1.5cm} >{\raggedleft}p{1.7cm} >{\raggedleft}p{1.1cm} >{\raggedleft}p{1.2cm} >{\raggedleft}p{1.2cm} >{\raggedright\arraybackslash\hspace{0pt}}X}
\toprule
\textbf{年份} & \textbf{出货\\(亿GB)} & \textbf{均价\\\$/GB} & \textbf{TAM\\(亿\$)} & \textbf{YoY\\(\%)} & \textbf{主力代(价值占)} & \textbf{核心驱动} \\
\midrule
...
\vspace{0.4em}
\noindent\textbf{投资含义}：HBM TAM 的指数级增长 (2024 \$74 亿 \(\to\) 2027 \$580--680 亿) 是半导体行业近十年来最陡峭的增长曲线之一。但 A 股无 HBM 原厂，\textbf{直接受益路径只有三条}：(1) 设备材料 (北方华创/中微/拓荆 — 扩产必买，确定性最高)；(2) 配套内存接口 (澜起 — HBM 涨价反而推升 CXL 替代需求)；(3) 先进封装 (长电\slash 通富 — 远期 HBM 封装国产化路径)。纯「国产 HBM 概念股」无基本面支撑，需规避。
...
\centering
\footnotesize
\begin{tabularx}{\exhibitboxwidth}{>{\bfseries}l >{\raggedleft}p{1.5cm} >{\raggedleft}p{1.5cm} >{\raggedleft}p{1.6cm} >{\raggedleft}p{1.5cm} >{\raggedright\arraybackslash}X}
\toprule
...
\midrule
\rowcolor{nvgreen!8}
\textbf{CXL (内存池化+SSD)} & 约8--12 & 约\textbf{35--45} & 约\textbf{100--130} & 约\textbf{120\%} & \textbf{澜起} \astockstar (CXL 2.0 控制器量产 + 3.0 送样；2026 收入 约10 亿元，2027 年翻倍)；江波龙 (CXL 模组研发中)；深科技 (代工储备) \\
\midrule
\rowcolor{panelgrey}
...
CXL (Compute Express Link) 标准通过在 CPU/GPU 与内存之间建立统一高速互连，实现跨节点的内存池化与扩展。对于 AI 数据中心，CXL 的核心价值在于——\textbf{当 HBM 成本过高或供给不足时，通过 CXL 扩展池化内存作为低成本替代方案}。从技术演进路径看，CXL 1.1 (2022) 仅支持基础 Cache/Mem 协议，CXL 2.0 (2023) 引入 Switch 实现多对多内存池化，而 CXL 3.0/3.1 (2025-2026) 进一步将带宽翻倍至 64GT/s (PCIe 6.0) 并支持 P2P 拓扑 (Fabric 互连)。Intel Emerald Rapids、AMD Genoa、Sapphire Rapids Refresh 三代服务器 CPU 已原生支持 CXL 2.0，NVIDIA Rubin 平台据传也将在 2026 年 H2 首次引入对 CXL 内存扩展的原生支持——这一信号将彻底改变 CXL 从「CPU 生态附属」向「AI/HBM 互补基础设施」跃迁的市场预期。
...
\begin{tikzpicture}[
  ...
  ashare/.style={rectangle, rounded corners=3pt, draw=riskred, fill=white, text width=3.0cm, align=center, minimum height=1.0cm, font=\bfseries\small, text=deepnavy, line width=0.9pt},
...
\node[ashare, fill=white, draw=riskred, text=deepnavy] (P2) at (3.8, -4.4) {Pool B · Type 3\\\astockstar\ \textbf{CXL MXC 控制器}\\澜起全球首发量产};
\node[pool] (P3) at (7.3, -4.4) {Pool C · Tiering\\热 DDR5 · 冷 CXL SSD\\Samsung · Solidigm};
\node[ashare, fill=white, draw=riskred, text=deepnavy] (P4) at (10.8, -4.4) {Pool D · 模组/SSD\\\astockstar\ \textbf{江波龙 / 深科技}\\CXL 模组研发中};

% Arrows
\foreach \h in {H1,H2,H3,H4} {
  \draw[line, accentblue] (\h.south) |- (SW1.north);
  \draw[line, dashed, accentblue!50] (\h.south) |- (SW2.north);
}
\node[midway, above, font=\tiny, color=accentblue] (L2) at (4.7, -1.1) {PCIe 5.0/6.0 · CXL 2.0/3.0 协议 · 32/64GT/s};
\draw[line, navy] (SW1.south) |- (P1.north);
\draw[line, riskred, line width=1.2pt] (SW1.south) |- (P2.north);
\draw[line, navy] (SW2.south) |- (P3.north);
\draw[line, riskred, line width=1.2pt] (SW2.south) |- (P4.north);
\node[midway, left, font=\tiny, color=riskred] (L3) at (5.1, -3.2) {\ensuremath{\bigstar}\ A 股核心卡位层};
\node[below=0.2cm of P2, font=\tiny, color=steelgrey] {CXL 2.0 已量产 · CXL 3.1 2026 年 H2 送样三星/AMD};
