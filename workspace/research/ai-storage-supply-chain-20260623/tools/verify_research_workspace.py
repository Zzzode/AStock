...
def verify_checksums() -> tuple[int, list[str]]:
    manifest = load_json("data/core_artifact_checksums_20260623.json")
    manifest_md = (BASE / "data/core_artifact_checksums_20260623.md").read_text(encoding="utf-8")
    problems: list[str] = []
    for item in manifest.get("files", []):
        ...
        if expected_md_row not in manifest_md:
            problems.append(f"{item['path']}: missing or stale row in core_artifact_checksums_20260623.md")
...
def verify_handoff_terms() -> list[str]:
    ...
    form_sd = (BASE / "data/customer_form_sd_conflict_minerals_recheck_20260623.md").read_text(encoding="utf-8")
    form_sd_terms = [
    ...
    for term in form_sd_terms:
        if term not in form_sd:
            problems.append(f"customer_form_sd_conflict_minerals_recheck_20260623.md missing {term}")
    return problems


def verify_root_inventory() -> tuple[int, list[str]]:
    root_inv = load_json("data/root_artifact_inventory_20260623.json")
    problems: list[str] = []
    for item in root_inv.get("files", []):
        ...
        root_md = (BASE / "data/root_artifact_inventory_20260623.md").read_text(encoding="utf-8")
        if expected_md_row not in root_md:
            problems.append(f"{item['file']}: missing or stale row in root_artifact_inventory_20260623.md")
    return len(root_inv.get("files", [])), problems


def verify_top_level_data_inventory() -> tuple[int, list[str]]:
    data_inv = load_json("data/top_level_data_artifact_inventory_20260623.json")
    data_md = (BASE / "data/top_level_data_artifact_inventory_20260623.md").read_text(encoding="utf-8")
    problems: list[str] = []
    for item in data_inv.get("files", []):
        ...
        if expected_md_row not in data_md:
            problems.append(f"{item['file']}: missing or stale row in top_level_data_artifact_inventory_20260623.md")
    return len(data_inv.get("files", [])), problems


def verify_source_inventory() -> tuple[int, list[str]]:
    source_inv = load_json("data/source_artifact_inventory_20260623.json")
    source_md = (BASE / "data/source_artifact_inventory_20260623.md").read_text(encoding="utf-8")
    problems: list[str] = []
    for item in source_inv.get("files", []):
        ...
        if expected_md_row not in source_md:
            problems.append(f"{item['path']}: missing or stale row in source_artifact_inventory_20260623.md")
...
def verify_render_directory_index(rendered_inv: dict) -> list[str]:
    ...
    stale_current = "rendered/full-20260623-0323` | 71 | Full current"
    if stale_current in index_text:
        problems.append("data_room_index.md labels full-20260623-0323 as current")
    return problems
...
def verify_directory_count_tables() -> list[str]:
    ...
    required_source_dirs = [
        "sources/probe-current-customer-supplier-lists-20260623",
        "sources/probe-customer-annual-risk-disclosures-20260623",
        "sources/probe-customer-form-sd-20260623",
        "sources/probe-upstream-supplier-lists-20260623",
        "sources/probe-hyperscaler-capex-20260623",
    ]
...
def verify_audit_markdown_summaries(refs: dict, blocker_coverage: dict, json_validity: dict) -> list[str]:
    ...
    refs_md = (BASE / "data/evidence_reference_integrity_audit_20260623.md").read_text(encoding="utf-8")
    refs_terms = [
    ...
    for term in refs_terms:
        if term not in refs_md:
            problems.append(f"evidence_reference_integrity_audit_20260623.md missing {term}")

    blocker_md = (BASE / "data/blocker_evidence_coverage_audit_20260623.md").read_text(encoding="utf-8")
    blocker_terms = [
    ...
    for term in blocker_terms:
        if term not in blocker_md:
            problems.append(f"blocker_evidence_coverage_audit_20260623.md missing {term}")

    json_md = (BASE / "data/json_validity_audit_20260623.md").read_text(encoding="utf-8")
    ...
    for term in json_terms:
        if term not in json_md:
            problems.append(f"json_validity_audit_20260623.md missing {term}")
...
def verify_consistency_markdown_summaries(
    ...
    blocker_md = (BASE / "data/blocker_request_pack_consistency_20260623.md").read_text(encoding="utf-8")
    ...
    for key, expected in expected_request_pack.items():
        if blocker_consistency.get(key) != expected:
            problems.append(f"blocker_request_pack_consistency_20260623.json {key} {blocker_consistency.get(key)!r}!={expected!r}")
    blocker_terms = [
    ...
    for term in blocker_terms:
        if term not in blocker_md:
            problems.append(f"blocker_request_pack_consistency_20260623.md missing {term}")

    source_md = (BASE / "data/source_exhaustion_consistency_20260623.md").read_text(encoding="utf-8")
    source_terms = [
    ...
    for term in source_terms:
        if term not in source_md:
            problems.append(f"source_exhaustion_consistency_20260623.md missing {term}")

    hygiene_md = (BASE / "data/pdf_text_hygiene_check_20260623.md").read_text(encoding="utf-8")
    hygiene_terms = [
    ...
    for term in hygiene_terms:
        if term not in hygiene_md:
            problems.append(f"pdf_text_hygiene_check_20260623.md missing {term}")

    path_md = (BASE / "data/pdf_path_leakage_check_20260623.md").read_text(encoding="utf-8")
    ...
    for term in path_terms:
        if term not in path_md:
            problems.append(f"pdf_path_leakage_check_20260623.md missing {term}")

    paid_md = (BASE / "data/paid_access_recheck_20260623.md").read_text(encoding="utf-8")
    for module in paid_access.get("modules", []):
        ...
        if expected_row not in paid_md:
            problems.append(f"paid_access_recheck_20260623.md missing {expected_row}")
    paid_terms = [
    ...
    for term in paid_terms:
        if term not in paid_md:
            problems.append(f"paid_access_recheck_20260623.md missing {term}")
...
def verify_customer_recheck_summaries(current_lists: dict, annual_risk: dict, form_sd: dict) -> list[str]:
    ...
    current_md = (BASE / "data/current_customer_supplier_list_recheck_20260623.md").read_text(encoding="utf-8")
    current_terms = [
    ...
    for term in current_terms:
        if term and str(term) not in current_md:
            problems.append(f"current_customer_supplier_list_recheck_20260623.md missing {term}")

    annual_md = (BASE / "data/customer_annual_risk_disclosure_recheck_20260623.md").read_text(encoding="utf-8")
    annual_terms = [
    ...
    for term in annual_terms:
        if term and str(term) not in annual_md:
            problems.append(f"customer_annual_risk_disclosure_recheck_20260623.md missing {term}")

    form_sd_md = (BASE / "data/customer_form_sd_conflict_minerals_recheck_20260623.md").read_text(encoding="utf-8")
    form_sd_terms = [
...
def verify_verifier_gate_descriptions() -> list[str]:
    ...
    data_room_json = load_json("data/data_room_index_integrity_audit_20260623.json")
    ...
    files_and_terms = {
        ...
        "data/data_room_index_integrity_audit_20260623.md": [
            "Ticker coverage matrix alignment",
            "audit Markdown summaries",
            "consistency Markdown summaries",
            "customer recheck Markdown summaries",
        ],
        "data/data_room_index_integrity_audit_20260623.json": [
    ...
    for rel, terms in files_and_terms.items():
        ...
        elif rel == "data/data_room_index_integrity_audit_20260623.json":
...
def main() -> int:
    ...
    data_room = load_json("data/data_room_index_integrity_audit_20260623.json")
    refs = load_json("data/evidence_reference_integrity_audit_20260623.json")
    root_inv = load_json("data/root_artifact_inventory_20260623.json")
    data_inv = load_json("data/top_level_data_artifact_inventory_20260623.json")
    source_inv = load_json("data/source_artifact_inventory_20260623.json")
    raw_inv = load_json("data/raw_data_artifact_inventory_20260623.json")
    rendered_inv = load_json("data/rendered_artifact_inventory_20260623.json")
    hygiene = load_json("data/pdf_text_hygiene_check_20260623.json")
    path_leak = load_json("data/pdf_path_leakage_check_20260623.json")
    ...
    raw_inventory_count, raw_inventory_problems = verify_simple_inventory(
        "data/raw_data_artifact_inventory_20260623.json",
        "data/raw_data_artifact_inventory_20260623.md",
        "data",
        "raw_data_files",
    )
    rendered_inventory_count, rendered_inventory_problems = verify_simple_inventory(
        "data/rendered_artifact_inventory_20260623.json",
        "data/rendered_artifact_inventory_20260623.md",
        "rendered",
        "rendered_files",
    )
    ...
    json_validity = load_json("data/json_validity_audit_20260623.json")
    quality = load_json("data/report_quality_eval.json")
    blocker_consistency = load_json("data/blocker_request_pack_consistency_20260623.json")
    blocker_coverage = load_json("data/blocker_evidence_coverage_audit_20260623.json")
    source_exhaustion_consistency = load_json("data/source_exhaustion_consistency_20260623.json")
    paid_access = load_json("data/paid_access_recheck_20260623.json")
    current_lists = load_json("data/current_customer_supplier_list_recheck_20260623.json")
    annual_risk = load_json("data/customer_annual_risk_disclosure_recheck_20260623.json")
    form_sd = load_json("data/customer_form_sd_conflict_minerals_recheck_20260623.json")
