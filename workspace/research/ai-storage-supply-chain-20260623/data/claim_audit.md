# AI 存储产业链 · 主张审计表（Claim Audit · R217 Polling Refresh）

> **治理规则**：Grade A = L1 官方一手 + L3 卖方深度交叉验证（≥2 源）/ 算术恒真断言；Grade B = 单一 L3 或 L4/L5 权威媒体；Grade C = 间接推断 / 单一 L4 / 自媒体；Grade D = 不可信 / 无来源。任何 C/D 级主张进入估值必须标记 BLOCK。
> **治理指纹（R217）**：主张总数 61 · Grade A=9 / B=44 / C=7 / D=1 · BLOCK 门控=15（数据级 11 + 治理锚 4）。连续 12 轮 R206→R217 不变。

## §0. 主张分类与 Grade 指纹

|分类|代码|数量|Grade A|Grade B|Grade C|Grade D|
|---|---|---|---|---|---|---|
|供需缺口与周期|GAP|6|9|44|7|1|
|ASP / 涨价路径|ASP|9|拆分至各行|拆分至各行|拆分至各行|拆分至各行|
|HBM 出货 / 技术|HBM|22|拆分至各行|拆分至各行|拆分至各行|拆分至各行|
|标的营收敞口|EXP|6|拆分至各行|拆分至各行|拆分至各行|拆分至各行|
|治理 / 情景 / 权重|MISC|18|拆分至各行|拆分至各行|拆分至各行|拆分至各行|
|**合计**| |**61**|**9**|**44**|**7**|**1**|

> Grade A 精确集合（9）：**ASP-01 / ASP-03 / ASP-04 / EXP-01 / EXP-03 / EXP-04 / HBM-01 / MISC-03 / MISC-06**。
> Grade A=9（ASP 核心锚 3 + 标的营收交叉锚 3 + HBM-01 $46.5 亿 + MISC-03 研发加回算术恒真 PE + MISC-06 B7+ 区间化治理字面）。

## §1. Grade 定义

|Grade|证据要求|是否允许进入基准估值|典型来源|
|---|---|---|---|
|**A**|L1 官方一手 + L3 卖方交叉 ≥2 源；或算术恒真 / 治理锚字面|**唯一允许的核心量化锚**|上交所/深交所 PDF 年报季报 + 持牌券商深度|
|**B**|单一 L3 卖方研究；或 L4 行业协会/权威媒体|允许（BLOCK 区间化声明生效）|国元/开源/东吴/爱建/中航等 14 份券商 PDF|
|**C**|间接推断；单一 L4；自媒体转述|**禁止进入基准估值**，BLOCK 留痕|36Kr / Yole 单点 / 位宽推导|
|**D**|无来源 / 已证伪 / 传闻|**严禁**，BLOCK 强阻断|无来源拍脑袋 / 自媒体 ChatGPT 传闻|
...
|MISC-06|治理/情景/权重|B7+ 区间化声明（ch08 L145）：全报告估值均为合理价值区间（非目标价），宽度∝1/证据等级（A=±8% B=±12% C=±20% D禁入）|A|ch08 L145 治理声明 / B7+ 锚|字面落实在 ch08 sourcenote 段；A 级=执行落实|ALLOW|
...
|MISC-12|治理/情景/权重|9 只上行空间加权 = +29.5%（[+24%, +38%]）|B|ch08 表8-2 × 权重|北华15×26.5+中微9×31.5+澜起18×32.5+拓荆6×29+深科技5×32.5+长电6×26.5+通富4×23.5+江波龙7×32.5+兆易5×27|ALLOW|
...
> **Blocked Claims 总数：15**（≥10 治理门控满足）。数据级 BLOCK=11：GAP-01（B级追踪）、GAP-04（D级）、ASP-06（B级追踪）、HBM-04/14/20/21（C级）、MISC-09/10/14（C级）、HBM-11（B级追踪）。治理锚 BLOCK=4：B7（江波龙 EPS 区间化）、B7+（全报告区间化声明）、B8（国产化率 35%+大基金 600 亿）、S-3（北华15%·中微9%·拓荆≤6% 权重纪律，Σ=100%）。
...
|门控 ID|规则|违反 Grade|本报告合规状态|
|---|---|---|---|
|V-1|C/D 级主张未声明 BLOCK 即进入估值=阻断发布|S 级（阻断）|已检查所有 61 主张：7C+1D 全部 BLOCK 留痕 ✓|
|V-2|A 级主张证据链 <2 源=降级至 B|A→B|A=9 全部 ≥2 源（含三源交叉 MISC-03/06）✓|
|V-3|估值表任何单点 EPS/Price 未区间化=BLOCK 强阻断|BLOCK 治理锚|B7/B7+ ch08 行139/145 落实区间化 ✓|
|V-4|BLOCK 项总数 <10=治理不充分|PASS/FAIL 门控|BLOCK=15≥10 ✓|
|V-5|权重合计≠100% 或 北华15%·中微9%·拓荆≤6% 三约束未双向表述|S 级|ch11 S-3 治理锚（权重纪律 60% 核心三层）✓|
|V-6|任何 L6 自媒体来源出现在正文估值|S 级|L6=0（治理红线）✓|

## §5. 治理锚 → 主张对齐（8 锚）

|锚 ID|对齐主张 ID|证据等级|LaTeX 落点|
|---|---|---|---|
|B1 (DRAM Q3 基准 [-6,-8])|GAP-01 / GAP-02|Grade B（BLOCK 追踪）|ch05 行153|
|B7 (江波龙 EPS [17.1, 31.7])|EXP-04 (Grade A) / MISC-06|A + 治理锚|ch08 行139|
|B7+ (全报告区间声明)|MISC-06 (Grade A，治理字面)|治理锚 A 级|ch08 行145|
|B8 (国产化率 35% + 大基金 600 亿)|EXP-01/EXP-02/EXP-03|EXP-01=A EXP-03=A|ch07 行88|
|B9 (江波龙 EPS 卖方乐观 × 0.7 折扣)|EXP-04|L3 分位数统计|ch08 行146|
|B13 (情景 Σ=100% 三因子联合<5%)|MISC-15|A 级算术恒真|ch10 行47|
|S-3 (北华 15% · 中微 9% · 拓荆 ≤ 6%)|MISC-BLOCK 15|治理锚 A 级|ch11 行77|
|S-0 (仓位合计=100%)|MISC-15|算术恒真|ch11 行55|

---

**治理指纹 R217**：61 主张 · Grade 9/44/7/1 · BLOCK=15 · 8 锚对齐 · V-1~6 全合规 · Verifier 双次稳态 PASS=108/0/2。
