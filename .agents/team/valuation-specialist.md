...
- Valuation recovery when direct broker targets are missing: public consensus, comparable companies, market-implied valuation, sentiment premium/discount and alternative methods.
...
3. Choose a primary valuation method and secondary check for each ticker before calculating target values.
   - If a ticker is low-base, turnaround, settlement-driven, project-cycle distorted or near-zero EPS, run the Valuation Recovery Loop before downgrading it.
...
8. Calculate bear/base/bull values, bubble degree, final multi-anchor target, implied upside/downside, catalysts, invalidation triggers, and next-quarter thresholds.
   - A missing direct broker target must trigger peer, consensus and market-implied checks, not an automatic stop.
