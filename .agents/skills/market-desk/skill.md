...
|---|---:|---|---|
| `ultra_short` | 1–3 trading days | Does the closing leader/event structure justify a conditional next-session plan? | `ultra-short-tactical-trader`, `execution-liquidity-analyst`; use complete daily price-limit ecology, breadth, leader, liquidity, catalyst, and a dated next-session plan. Reproducible auction/L2/trade data is required only for auction, queue, absorption, or order-flow claims. |
...
|---|---|---|---|---|
| `theme_ignition_first_board` | 1--3 days. A newly disclosed catalyst begins to concentrate attention in a coherent theme. | Timestamped catalyst source; complete closing price-limit ecology, market breadth, sector membership, leader identity, liquidity, and a dated next-session conditional plan. | Confirm only when the catalyst, theme breadth, closing leader, and price-limit ecology agree; invalidate on false/unrelated catalyst, isolated move, or failed participation; expire at the next-session review if follow-through does not appear. | `ultra-short-tactical-trader`, `sector-rotation-analyst`, `event-driven-institutional-analyst`, `execution-liquidity-analyst`. |
| `leader_continuation` | 1--3 days. A recognized leader retains scarce attention after the market has had a chance to disagree. | Current closing leader hierarchy, price-limit ecology, theme breadth, turnover/liquidity, and a dated next-session conditional plan. Reproducible intraday data is required only for an auction, queue, absorption, or order-flow assertion. | Confirm only when leadership survives disagreement into the close while the theme remains participatory; invalidate when leadership transfers or participation collapses; expire after the next tradable review window. | `ultra-short-tactical-trader`, `sector-rotation-analyst`, `execution-liquidity-analyst`, `risk-analyst`. |
...
price-limit ecology, trading-status and leadership coverage plus dated
next-session entry, rejection, and review conditions. Reproducible intraday
execution evidence is required only when making a microstructure claim. A
source cannot be substituted by an indicator or by an unsourced price interpretation.
...
4. Select a book-specific specialist: `ultra-short-tactical-trader` for EOD-to-next-session conditional plans, `short-term-trader`, `swing-trend-analyst`, or `event-driven-institutional-analyst`. Require reproducible intraday data only if that specialist makes an auction, queue, absorption, or order-flow claim.
