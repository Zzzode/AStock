...
for p in srcs:
    ...
    if (s,z)==exp: ok+=1
    else: print(f"MISMATCH {fn}"); sys.exit(1)
assert ok==15
print(f"[3a] 15 SHA·Size 15/15")
...
sr_md=sr_md.replace(f"R{LAST} Polling Refresh · 第{LAST-205}轮", f"R{NEW} Polling Refresh · 第{RND}轮")           .replace(f"R206→R{LAST} 连续 {LAST-205} 轮幂等", f"R206→R{NEW} 连续 {RND} 轮幂等")           .replace(f"R206→R{LAST} 连续 {LAST-205} 轮锁定", f"R206→R{NEW} 连续 {RND} 轮锁定")           .replace(f"治理指纹连续 {LAST-205} 轮锁定 SR=54", f"治理指纹连续 {RND} 轮锁定 SR=54")           .replace(f"治理指纹（R{LAST}）", f"治理指纹（R{NEW}）")
...
print(f"[3d] SR R{LAST}→R{NEW} rows={len(rows_old)} zero")
sr_j=json.loads((DATA/"source_registry.json").read_text(encoding="utf-8"))
assert sr_j.get("polling_round")==f"R{LAST}"
sr_j["polling_round"]=f"R{NEW}"; sr_j["generated_at"]=datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
...
print(f"[3e] records={len(records)} phys={pn} qhash={qhash}")
ca=(DATA/"claim_audit.md").read_text(encoding="utf-8")
ca_N=max([int(m.group(1)) for m in re.finditer(r"R(\d+) Polling Refresh", ca)] + [LAST])
assert ca_N==LAST
ca=ca.replace(f"Claim Audit · R{LAST} Polling Refresh · 第{LAST-205}轮", f"Claim Audit · R{NEW} Polling Refresh · 第{RND}轮")      .replace(f"R206→R{LAST} 连续 {LAST-205} 轮锁定", f"R206→R{NEW} 连续 {RND} 轮锁定")      .replace(f"R206→R{LAST} 连续 {LAST-205} 轮幂等", f"R206→R{NEW} 连续 {RND} 轮幂等")      .replace(f"治理指纹（R{LAST}）", f"治理指纹（R{NEW}）")      .replace(f"治理指纹 R{LAST}", f"治理指纹（R{NEW}）")
...
print(f"[3f] Grade={gd} Σ=61 BLOCK={blk}")
...
print(f"[4幂等] d1={d1} d2={d2} d3={d3} d4={d4}")
print("Step3 OK")
