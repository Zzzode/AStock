---
name: data-collector
description: Structured financial and market data gathering from official filings and exchanges
...
- **Corporate events / catalysts (announcements, earnings, dividends):** PREFER `.venv/bin/python -m astock.cli news <code> [--days N] [--json]` (verified working — returns up to N recent dated events with title, summary, source, url, severity; add `--json` for structured parsing). Fall back to eastmoney public feeds only if the CLI is unavailable.
